...
def box_trace(lower, upper, draw_faces=False, line=None, linewidth=None,
              ...
              intensity=None, hovertext=None, showscale=False, **kwargs):
    ...
    **kwargs : dict, optional
    ...
    if not draw_faces:
        ...
        if (color is not None or linewidth is not None or
            cmin is not None or cmax is not None or colorscale is not None):
            assert line is None, (
        ...
        trace = go.Mesh3d(x=vertices[:, 0], y=vertices[:, 1], z=vertices[:, 2],
                          i=box_tri_index[0], j=box_tri_index[1],
...
def scatter_boxes(coords, dimension, draw_faces=True, color='orange',
                  hovertext=None, linewidth=2, shared_legend=True, **kwargs):
    """Function which produces a list of plotly traces of boxes given a list of
    ...
    **kwargs : dict, optional
    ...
    if not np.isscalar(dimension):
        assert len(dimension) == 3, (
                "Must specify three dimensions for the box size.")
    ...
    return box_traces(
            lowers, uppers, draw_faces, color, linewidth, hovertext,
            shared_legend, **kwargs)
